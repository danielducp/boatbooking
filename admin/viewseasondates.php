...
while($row = $stmt->fetch())
{
		 ...
		 "<TD align=center>".$row['SeasonDatesID']."</TD>";
    $SeasonDatesID = $row['SeasonDatesID'];
	 	$SeasonStartDate = $row['SeasonStartDate'];
	$SeasonEndDate = $row['SeasonEndDate'];

    ?>
<div class="card">
Season Start Date: <?php echo $SeasonStartDate?><br>
Season End Date: <?php echo $SeasonEndDate?> 

<form action="editdate.php" method="post">

<input type="hidden" name="SeasonDatesID" value="<?=$SeasonDatesID?>">

<input type="hidden" name="SeasonStartDate" value="<?=$SeasonStartDate?>">
          <input type="hidden" name="SeasonEndDate" value="<?=$SeasonEndDate?>">
    <?php
  }

?>

<input type="submit" value="Edit me" class="btn btn-info"></input>
