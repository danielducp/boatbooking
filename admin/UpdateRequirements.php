...
$sqlQuery->bindParam(':ExtraRequirements', $_POST['ExtraRequirements']); 


$sqlQuery->bindParam(':BookingNumber', $_POST['BookingNumber']); 

$sqlQuery->execute(); 

echo "<h1  align=center>Requirements updated</h1>";


?>
    <form action="viewsailingbookingsdateselected.php" method="post" >
            <input type="hidden" name="BookingNumber" value="<?=$BookingNumber?>">
            <input type="hidden" name="ChosenDate" value="<?=$ChosenDate?>">

            <input name="submit" value="Click to go back" class="btn btn-info" type="submit">
</td>    
</form> 
