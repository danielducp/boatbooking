...
<br>
  ...
  $AmountOfBoats;
      ...
      <?php $mostrecentbooking = $product['BookingNumber'];?>
              </div>
          
            </form> 
           
            <?php  $mostrecentbooking+1?>         
                <?php endforeach; 
        ...
        ?>
                          ...
                          <div class="card-group">
                <?php foreach ($recently_added_products as $product): ?>                   
      <?php $mostrecentbookedboatid = $product['BookedBoatsID'];?>
              </div>
          
            </form> 
           
            <?php  $mostrecentbookedboatid+1?>         
                <?php endforeach; ?>
...
$EndTime =  date('Y-m-d H:i:s', $newendtime); 
  ...
  $sqlQuery = $pdo->query("SELECT max(BookedBoats.BoatNumber) AS total FROM AvailableBoat INNER JOIN BookedBoats ON AvailableBoat.BoatNumber = BookedBoats.BoatNumber WHERE BookedBoats.StartTime = '$StartTime' AND BookedBoats.BoatID = '$chosenboat' ");
  $row6=$sqlQuery->fetch();
  $Test2 = $row6['total'];
   "<br>";
   "Total Boats:";
 ...
 "<br>";
            ...
            foreach (range($x, $AmountOfBoats) as $number) {
              ...
              "<br>";         
               ...
               "testing";
               $BoatNumberAssigned = ($Test2 + $number);
               "<br>";         
...
$RecentBookingNumber = $NRecentBookingNumber+1;
 $RecentBookingNumber;
 $ArrivalStatusName ="N/A";



 $query = "INSERT INTO `Booking` (BookingNumber, BookingCreationDate, BookingSailingDate, ConfirmationSent, ReminderSent, Duration, UserID, QuantityBooked, ExtraRequirements, ArrivalStatusName) 
 VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
$stmt = $pdo->prepare($query);
$stmt->execute([$BookingNumber, $BookingCreationDate, $newformatdate, $ConfirmationSent, $ReminderSent, $Duration, $UserID, $AmountOfBoats, $boatextrarequirements, $ArrivalStatusName]);
...
$sqlQuery = $pdo->query("SELECT * FROM User WHERE UserID = '$UserID'");
$row81=$sqlQuery->fetch();
$email = $row81['Email'];



$sqlQuery = $pdo->query("SELECT * FROM Boat WHERE BoatID = '$chosenboat'");
$row82=$sqlQuery->fetch();
$BoatName = $row82['BoatName'];
          $chosentimewithoutseconds=substr($chosentime,0,-3);

   $newDate = date("l d m Y", strtotime($chosendate));  

 $email;
$message = "Thank you for booking your sailing session. You have successfully booked $AmountOfBoats $BoatName for $newDate at $chosentimewithoutseconds. Please arrive 15 minutes before your allocated time to allow for check in.

If you are unable to sail please ensure you cancel your session by viewing your booking on line or contact Peterborough Sailability via Phone or Email.

On rare occasions we do need cancel the whole days sailing due to adverse weather conditions, on these occasions we will send you an email to advise you. Please check your emails prior to departing to avoid disappointment when you arrive.
...
";
   ...
   // Send
    mail($email, 'Booking Confirmation', $message);
